...
TokenService::TokenService(QSettings *settings, Api*api, QObject* parent) :
    QObject(parent),
    settings(settings),
    api(api)
{
...
void TokenService::setAccessToken(const QString &value)
{
    if(accessToken != value){
        accessToken = value;
        qDebug().nospace().noquote() << "accessToken";
...
void TokenService::setRefreshToken(const QString &value)
{
    if(refreshToken != value){
        refreshToken = value;
        qDebug().nospace().noquote() << "refreshToken";
